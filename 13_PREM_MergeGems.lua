...
do
    ...
    local TASelected = { mode = "id", guid = nil, name = nil }
    ...
    --  RESPIRATION SKILL SPAM (dipakai RANDOM ATTACK & TARGET ATTACK) 
    -- Nambahin remote PlayerRespirationSkillAttack (5 skillId) sebagai serangan
    -- tambahan, di-spam tiap 0.5 detik ke enemyGuid yang SEDANG diserang (dinamis,
    -- ngikut target RA/TA saat ini -- bukan guid hardcode).
    local _RESPIRATION_SKILL_IDS = {200730, 200731, 200732, 200733, 200734}

    local function FireRespirationSkillSpam(g)
        if not g then return end
        local remote = Remotes:FindFirstChild("PlayerRespirationSkillAttack")
        if not remote then return end
        for _, sid in ipairs(_RESPIRATION_SKILL_IDS) do
            task.spawn(function()
                pcall(function()
                    remote:InvokeServer({
                        enemyGuid       = g,
    ...
    local _respSpamThreads = {}

    local function StartRespirationSpamFor(g)
        if not g then return end
        if _respSpamThreads[g] and _respSpamThreads[g].running then return end
        local handle = {running = true}
        _respSpamThreads[g] = handle
        task.spawn(function()
            while handle.running do
                if IsEnemyGuidValid(g) then
                    FireRespirationSkillSpam(g)
                end
                task.wait(0.5)
            end
        end)
    ...
    local function StopRespirationSpamFor(g)
        if g and _respSpamThreads[g] then
            _respSpamThreads[g].running = false
            _respSpamThreads[g] = nil
        end
    end

    local function StopAllRespirationSpam()
        for _, handle in pairs(_respSpamThreads) do
    ...
    local function TaSpamF(g, enemyHRP)
        ...
        -- [RESPIRATION SPAM] Serangan tambahan PlayerRespirationSkillAttack, tiap 0.5 detik
        StartRespirationSpamFor(g)
    end
    ...
    local function StopRA()
        ...
        -- Stop semua respiration-spam thread (jaga-jaga guid tertinggal)
        StopAllRespirationSpam()
        ...
        local tMain = task.spawn(function()
            ...
            while RA.running do
                if not RA.cur or not IsTargetAliveRA(RA.cur) then
                    ...
                    if _oldGuid then StopRespirationSpamFor(_oldGuid) end
