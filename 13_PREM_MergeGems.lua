...
do
    ...
    local GEM_MERGE_MIN_LEVEL = 22  -- level di bawah ini di-exclude (belum relevan utk merge)

    -- Tabel referensi iconId -> nama gem, HANYA berisi iconId yang sudah
    -- dikonfirmasi manual oleh user (warna merah=Ruby, hijau=Emerald,
    -- biru=Sapphire). iconId gem lain (levelnya belum pernah discan user,
    -- atau bukan Ruby/Emerald/Sapphire) SENGAJA tidak dimasukkan -- supaya
    -- otomatis disembunyikan dari dropdown Merge sampai user scan & tambah
    -- entry-nya di sini nanti.
    local GEM_ICON_NAME_MAP = {
        -- Sapphire (iconId hasil SCAN langsung dari inventory user)
        ["rbxassetid://99944406692209"]  = "Sapphire", -- Lv.28
        ["rbxassetid://120786278511706"] = "Sapphire", -- Lv.29
    }
    ...
    local function ScanAndGroupMG()
        ...
        for _, item in ipairs(raw) do
            ...
            -- Hanya gem yang: (a) levelnya di range 22-29, DAN (b) iconId-nya
            -- dikenal di GEM_ICON_NAME_MAP (dijamin Ruby/Emerald/Sapphire).
            -- Gem lain (level di luar range, atau jenis selain 3 itu) otomatis
            -- disembunyikan dari dropdown Merge.
            if item.level >= GEM_MERGE_MIN_LEVEL and item.level < GEM_MERGE_MAX_LEVEL and knownName then
